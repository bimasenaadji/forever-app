  ...
  return (
    <article>
      <div className="grid grid-cols-1 grid-flow-row lg:grid-cols-2">
        <Header product={product} className={"h-[90%]"} />
        <main>
          <Body
            product={product}
...
const Header = ({ product, className }) => {
  return (
    <img className={className} src={product.imageUrl} alt="Product Image" />
  );
};
...
const Body = ({ product, onAddToCart, setSelectedSize, selectedSize }) => {
  return (
    <div>
      ...
      <p className="font-medium text-[32px] my-3">Rp. {product.price}</p>
      <p className="font-normal text-desc text-base my-3">
        {product.description}
      </p>
      <p className="text-size font-semibold text-xl my-2 ">Select Size</p>
      <div className="text-base font-normal flex gap-x-[16px]">
        {["S", "M", "L", "XL", "XXL"].map((size) => (
          <button
            key={size}
            onClick={() => setSelectedSize(size)}
            className={`p-5 border border-border-input cursor-pointer ${
      ...
      <button
        onClick={onAddToCart}
        className="uppercase py-6 px-28 font-semibold text-base bg-black text-white my-5"
      >
        Add To Cart
      </button>
    </div>
